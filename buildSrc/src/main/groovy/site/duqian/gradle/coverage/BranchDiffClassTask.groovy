...
import site.duqian.gradle.util.CommonUtils
 ...
 * @author  Created by 杜小菜 on 2021/9/16 - 17:52 .
...
class BranchDiffClassTask extends DefaultTask {
    ...
    private def pullDiffClasses() {
        currentName = JacocoUtils.getCurrentBranchName()
        println "$TAG currentName:" + currentName + ",baseBranch=$jacocoExtension.branchName"
        //获得两个分支的差异文件
        ...
        FileUtil.copyFile(diffFile, new File(JacocoUtils.getUploadRootDir(project) + File.separator + diffFile.getName()))
    ...
    private void handleDiffClasses(String currentName, List<String> diffFiles) {
        ...
        def branchDir = "${project.rootDir.parentFile}/temp/${jacocoExtension.branchName}/${TEMP_DIR}"
...
//
    ...
    private void copyBranchClass(String currentName, GString currentDir) {
        ...
        if (CommonUtils.isWindowsOS()) {
            cmds = new String[5]
            cmds[0] = jacocoExtension.getGitBashPath()
            cmds[1] = jacocoExtension.pullDiffClassShell
            cmds[2] = currentName
            cmds[3] = project.rootDir.getAbsolutePath()
            cmds[4] = currentDir.toString()
        } else {
            cmds = new String[4]
            cmds[0] = jacocoExtension.pullDiffClassShell
            cmds[1] = currentName
